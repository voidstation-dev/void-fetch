...
} from "../src/infrastructure/hls-browser-download.ts";
...
describe("hls browser download helpers", () => {
  it("picks the highest bandwidth variant from a master playlist", () => {
    ...
    expect(
      pickBestVariant(playlist, "https://example.com/master.m3u8"),
    ).toEqual({
      bandwidth: 2400000,
  ...
  it("always exports browser hls downloads with an mp4 extension", () => {
    expect(
      inferHlsOutputExtension(null, [
        { url: "https://example.com/video/seg-000.ts" },
      ]),
    ).toBe("mp4");

    expect(
      inferHlsOutputExtension("https://example.com/video/init.mp4", [
