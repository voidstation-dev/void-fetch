...
describe("result card visibility helpers", () => {
  ...
  it("falls back to download urls when object-based image payloads omit direct urls", () => {
    expect(
      ...
      "https://download.example.com/1.jpg",
      "https://download.example.com/2.jpg",
    ]);
  });
  ...
  it("shows video download button for bilibili tv when a video url exists", () => {
    expect(shouldShowVideoDownloadButton("https://example.com/video.mp4")).toBe(
      true,
    );
  });
  ...
  it("uses frontend image proxy for ordinary remote images", () => {
    expect(
      shouldUseFrontendImageProxy(
        "https://wx3.sinaimg.cn/large/mock-cover.jpg",
      ),
    ).toBe(true);
  });
  ...
  it("skips frontend image proxy for backend image-proxy urls", () => {
    expect(
      shouldUseFrontendImageProxy(
        "http://localhost:8787/api/image-proxy?platform=weibo&url=https%3A%2F%2Fwx3.sinaimg.cn%2Flarge%2Fmock-cover.jpg",
      ),
    ).toBe(false);
  });
  ...
  it("skips frontend image proxy for backend indexed image download urls", () => {
    expect(
      shouldUseFrontendImageProxy(
        "http://localhost:8787/api/wechat/download?url=https%3A%2F%2Fmp.weixin.qq.com%2Fs%2Fabc&index=0&filename=test",
      ),
    ).toBe(false);
  });
