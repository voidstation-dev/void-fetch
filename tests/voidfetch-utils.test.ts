...
import {
  ...
  normalizeUrl,
  detectPlatform,
} from "../src/features/batch-download/utils/normalize-url";
...
describe("URL Normalizer & Platform Detector", () => {
  it("detects correct platforms from domains", () => {
    expect(detectPlatform("https://www.youtube.com/watch?v=dQw4w9WgXcQ")).toBe(
      "youtube",
    );
    expect(detectPlatform("https://www.bilibili.com/video/BV1GJ411x7h7")).toBe(
      "bilibili",
    );
    expect(detectPlatform("https://v.douyin.com/abc")).toBe("douyin");
    expect(detectPlatform("https://www.tiktok.com/@creator/video/123")).toBe(
      "tiktok",
    );
    expect(
      detectPlatform("https://www.xiaohongshu.com/discovery/item/123"),
    ).toBe("xiaohongshu");
  ...
  it("removes tracking parameters during URL normalization", () => {
    const raw =
      "https://www.bilibili.com/video/BV1GJ411x7h7?spm_id_from=333.1007&vd_source=123";
    expect(normalizeUrl(raw)).toBe(
      "https://www.bilibili.com/video/BV1GJ411x7h7",
    );

    const utm =
      "https://youtube.com/watch?v=123&utm_source=twitter&utm_medium=social";
...
describe("Filename Template Rendering", () => {
  it("replaces placeholder tokens correctly", () => {
    ...
    expect(rendered).toBe(
      "005 - bilibili_BV1GJ411x7h7 - Cool Video - Stream.mp4",
    );
  });
});
