...
export function getPlatformBadgeStyle(platform?: string): string {
  ...
  const config =
    PLATFORMS[p] || Object.values(PLATFORMS).find((c) => p.includes(c.id));

  return (
    config?.ui.badgeColor || "bg-primary/10 text-primary border-primary/20"
  );
}
...
export function getPlatformLabel(
  ...
  const get =
    typeof t === "function"
      ...
      : (k: string) =>
          (
            (t as Pick<Dictionary, "history">).history.platforms as Record<
              string,
              string
            >
          )[k] ?? k;
...
export function supportsAudioExtraction(
  platform: string | null | undefined,
): boolean {
