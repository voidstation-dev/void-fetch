...
import {
  fileSave,
  supported as supportsStreamingFileSave,
...
async function fetchProxyText(
  ...
  if (!response.ok) {
    throw new HttpStatusError(
      response.status,
      `Proxy request failed with HTTP ${response.status}`,
    );
  }
...
async function fetchDirectText(
  ...
  if (!response.ok) {
    throw new HttpStatusError(
      response.status,
      `Direct request failed with HTTP ${response.status}`,
    );
  }
...
async function fetchTextWithFallback(
  ...
  const directFetchMode = host
    ? (directFetchModes.get(host) ?? "probe")
    : "probe";
...
async function resolvePlaylist(
  ...
  if (/\.m3u8(?:[?#]|$)/i.test(sourceUrl)) {
    ...
    playlistUrl =
      parsed.data.originDownloadVideoUrl || parsed.data.downloadVideoUrl || "";
...
async function probeHostDirectAccess(
  ...
  const { signal: probeSignal, cleanup } = createProbeSignal(
    signal,
  ...
  try {
    ...
    if (!response.ok) {
      throw new HttpStatusError(
        response.status,
        `Direct probe failed with HTTP ${response.status}`,
      );
    }
...
async function probePlaylistHosts(
  ...
  await runWithConcurrency(
    ...
    async (probeTarget) => {
      ...
      directFetchModes.set(
        probeTarget.host,
        directAccessible ? "direct-ok" : "proxy-only",
      );
    },
  );
}
          ...
          await runWithConcurrency(
            ...
            async (target, index) => {
              ...
              if (target.keyUrl) {
                ...
                outputChunk = await decryptAes128Cbc(
                  bytes,
                  cryptoKey,
                  target.iv,
                );
...
export function HlsBrowserDownloadPanel({
  ...
  const [speedBytesPerSecond, setSpeedBytesPerSecond] = useState<number | null>(
    null,
  );
  ...
  const handleStart = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const extension = inferHlsOutputExtension(
        resolution.mapUrl,
        resolution.selectedSegments,
      );
      const baseTitle = sanitizeFilename(
        initialTitle || resolution.title || tHistory("unknownTitle"),
      );
      ...
      const response = createStreamingDownloadResponse({
        ...
        onChunkDownloaded: (bytes) => {
          ...
          const now = Date.now();
          downloadSamplesRef.current = [
            ...downloadSamplesRef.current.filter(
              (sample) => now - sample.timestamp <= 8000,
            ),
            { bytes, timestamp: now },
          ];
