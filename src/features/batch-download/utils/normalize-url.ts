...
export function detectPlatform(urlStr: string): string {
  try {
    ...
    if (host.includes('bilibili.com') || host.includes('bili.live') || host.includes('b23.tv')) return 'bilibili';
 ...
 */
export function extractAndNormalizeUrls(text: string, existingUrls: string[] = []): ExtractedUrl[] {
  if (!text) return [];
  ...
  for (const rawUrl of foundUrls) {
    ...
    if (platform === 'unknown') {
      status = 'malformed';
    } else if (seenInBatch.has(normalized) || existingUrls.includes(normalized)) {
      status = 'duplicate';
    }
