...
function isSupportedVideoFile(file: File): boolean {
  ...
  return (
    !!file.type &&
    (SUPPORTED_VIDEO_TYPES.has(file.type) || file.type.startsWith("video/"))
  );
}
...
function isSupportedAudioFile(file: File): boolean {
  ...
  return (
    !!file.type &&
    (SUPPORTED_AUDIO_TYPES.has(file.type) || file.type.startsWith("audio/"))
  );
}
...
function resolveResultTaskAction(
  task: AudioExtractTask | null | undefined,
): ResultTaskAction | null {
  if (!task) {
    return null;
  }
...
export function AudioExtractDialog({
  ...
  const validateMergeTotalSize = useCallback(
    (videoFile: File | null, audioFile: File | null): boolean => {
  ...
  const validateExtractVideoFile = useCallback(
    (file: File): boolean => {
  ...
  const validateMergeVideoFile = useCallback(
    (file: File): boolean => {
      if (file.size > MAX_VIDEO_FILE_SIZE) {
  ...
  const validateMergeAudioFile = useCallback(
    (file: File): boolean => {
  ...
  const statusText = (() => {
    ...
    return (entry === "result" && resultTaskAction === "merge-video") ||
      mode === "merge"
  ...
  const runAutoExtractTask = useCallback(
    async (task: AudioExtractTask) => {
      if (status === "error") {
        reset();
      }
      ...
      try {
        ...
        const audioDownloadUrl =
          parsed.downloadAudioUrl || parsed.originDownloadAudioUrl || null;
        const videoDownloadUrl =
          parsed.downloadVideoUrl || parsed.originDownloadVideoUrl || null;
        ...
        if (
          !resolvedTaskAction &&
          parsedActions.audioAction === "direct-download" &&
          audioDownloadUrl
        ) {
          setStage("direct-downloading");
  ...
  const handleExtractFileSelect = useCallback(
  ...
  const handleExtractFileDrop = useCallback(
  ...
  const handleMergeVideoSelect = useCallback(
    (event: ChangeEvent<HTMLInputElement>) => {
      const file = event.target.files?.[0];
      if (!file) return;
      ...
      if (
        !validateMergeVideoFile(file) ||
        !validateMergeTotalSize(file, mergeAudioFile)
      ) {
        event.target.value = "";
        return;
      }
  ...
  const handleMergeAudioSelect = useCallback(
    (event: ChangeEvent<HTMLInputElement>) => {
      const file = event.target.files?.[0];
      if (!file) return;
      ...
      if (
        !validateMergeAudioFile(file) ||
        !validateMergeTotalSize(mergeVideoFile, file)
      ) {
        event.target.value = "";
        return;
      }
  ...
  const handleMergeVideoDrop = useCallback(
    (event: DragEvent<HTMLDivElement>) => {
      event.preventDefault();
      event.stopPropagation();
  ...
  const handleMergeAudioDrop = useCallback(
    (event: DragEvent<HTMLDivElement>) => {
      event.preventDefault();
      event.stopPropagation();
  ...
  return (
    <AudioExtractDialogView
      ...
      runAutoExtractTask={() => {
        if (autoExtractTask) void runAutoExtractTask(autoExtractTask);
      }}
    />
  );
}
