...
import { DeferredHlsDownloadDialog } from "@/features/hls/components/deferred-hls-download-dialog";
...
function buildPageScopedResult(
  result: ResultData,
  page: NonNullable<ResultData["pages"]>[number],
): ResultData {
  return {
    ...result,
    ...
    originDownloadVideoUrl:
      page.downloadVideoUrl ?? result.originDownloadVideoUrl ?? null,
    originDownloadAudioUrl:
      page.downloadAudioUrl ?? result.originDownloadAudioUrl ?? null,
    mediaActions: undefined,
    currentPage: page.page,
  };
}

function buildVideoScopedResult(
  result: ResultData,
  video: NonNullable<ResultData["videos"]>[number],
): ResultData {
  return {
    ...result,
    ...
    downloadVideoUrl:
      video.downloadVideoUrl ?? video.originDownloadVideoUrl ?? null,
    downloadAudioUrl:
      video.downloadAudioUrl ?? video.originDownloadAudioUrl ?? null,
    originDownloadVideoUrl:
      video.originDownloadVideoUrl ?? video.downloadVideoUrl ?? null,
    originDownloadAudioUrl:
      video.originDownloadAudioUrl ?? video.downloadAudioUrl ?? null,
...
export function VideoResultPanel({
  ...
  const [hlsDownloadRequest, setHlsDownloadRequest] =
    useState<HlsDownloadDialogRequest | null>(null);

  const activeBiliList =
    activeBiliListState.key === activeListKey
  ...
  const derived = useMemo(() => {
    ...
    const pageTabLabel = tResult("totalParts", {
      count: result.pages?.length || 0,
    });
    const seasonTabLabel = tResult("videoCount", {
      count: result.videos?.length || 0,
    });
    const showMultiPartList =
      isMultiPart && (!hasBilibiliSourceSwitch || activeBiliList === "pages");
    const showSeasonList =
      hasEmbeddedVideos &&
      (!isMultiPart ||
        (hasBilibiliSourceSwitch && activeBiliList === "season"));
    const hasSupplementalImages = !isImageNote && displayImages.length > 0;
    const coverUrl =
      typeof result.cover === "string" ? result.cover.trim() : "";
    const shareSourceUrl =
      typeof result.url === "string" ? result.url.trim() : "";
    const canSharePlayLink =
      shareSourceUrl.length > 0 && canSharePlayResult(result);
  ...
  const selectedPageNumber =
    selectionState.key === activeListKey
  ...
  const selectedCoverUrl =
    activeCollectionSource === "season" && currentVideo?.cover
      ? currentVideo.cover.trim()
      : coverUrl;
  const coverSrc =
    selectedCoverUrl.length > 0 ? resolveCoverSrc(selectedCoverUrl) : "";
  ...
  const playerUrl =
    hlsPlaybackUrl ||
    (playerPreview ? buildMediaPreviewUrl(playerPreview) : null);
  const previewItem =
  ...
  const handleSelectPage = (pageNumber: number) => {
    ...
    setSelectionState((previous) => ({
      ...
      currentItemId:
        previous.key === activeListKey
          ? previous.currentItemId
          : result.currentItemId,
    }));
  };
            ...
            <div className="overflow-hidden rounded-lg bg-black">
              {playerPreview.mediaType === "audio" ? (
                ...
                <HlsVideoPlayer
                  ...
                  autoPlay={playerPreview.autoplay}
                  muted={
                    playerPreview.origin === "share" && playerPreview.autoplay
                  }
                  playsInline
                  preload="metadata"
                  poster={coverSrc || undefined}
                  className="w-full min-h-60 max-h-[50vh] bg-black"
                />
              ) : (
                <video
                  key={playerUrl}
                ...
                <div className="space-y-2">
                  {hasBilibiliSourceSwitch && (
                    <div className="flex items-center gap-2">
                      <Button
                        variant={
                          activeBiliList === "pages" ? "default" : "outline"
                        }
                        size="sm"
                      ...
                      <Button
                        variant={
                          activeBiliList === "season" ? "default" : "outline"
                        }
                        size="sm"
                        className="h-8 text-xs"
