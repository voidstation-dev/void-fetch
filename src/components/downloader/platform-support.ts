...
const DEFAULT_PLATFORM_ENTRIES: Record<PlatformSupportKey, PlatformSupportEntry> = {
...
export function getPlatformSupportItems(dict?: PlatformSupportDictionary | null): PlatformSupportItem[] {
    const support = dict?.guide?.platformSupport;

    const getItem = (key: PlatformSupportKey): PlatformSupportItem => {
        const entry = (support && (support as unknown as Record<string, PlatformSupportEntry>)[key]) || DEFAULT_PLATFORM_ENTRIES[key];
