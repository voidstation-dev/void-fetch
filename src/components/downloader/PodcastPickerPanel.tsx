...
export function PodcastPickerPanel({
  ...
  const initialEpisodeId =
    result.currentEpisodeId ?? result.currentItemId ?? episodes[0]?.id ?? "";
  ...
  const audioUrl = selectedEpisode
    ? selectedEpisode.downloadAudioUrl ||
      selectedEpisode.originDownloadAudioUrl ||
      null
    : null;
  ...
  const episodeAsResult: ResultData = selectedEpisode
    ? {
        ...
        mediaActions: {
          video: "hide",
          audio: audioUrl ? "direct-download" : "hide",
        },
      }
    : result;
  ...
  const canSharePlayLink =
    shareSourceUrl.length > 0 &&
    Boolean(selectedEpisode) &&
    canPreviewEpisodeAudio(selectedEpisode!);
  ...
  const activePlayerUrl = effectivePreview
    ? buildMediaPreviewUrl(effectivePreview)
    : null;
  const autoPlayActive =
    effectivePreview === activePreview && (activePreview?.autoplay ?? false);

  const coverUrl = selectedEpisode?.cover ?? result.cover ?? "";
  const coverSrc =
    typeof coverUrl === "string" && coverUrl.trim()
      ? resolveCoverSrc(coverUrl.trim())
      : "";
