...
export function PodcastEpisodeList({
  ...
  const autoScrollIndex = useMemo(
    () =>
      filteredEpisodes.findIndex(
        (episode) => episode.id === autoScrollEpisodeId,
      ),
  ...
  const {
    ...
    filteredEpisodes,
    autoScrollIndex >= 0
      ? Math.max(DEFAULT_VISIBLE_EPISODES, autoScrollIndex + 1)
      : DEFAULT_VISIBLE_EPISODES,
  );
      ...
      >
        <div className="space-y-2 pr-2">
          ...
          {visibleEpisodes.map((episode, index) => {
            const audioUrl =
              episode.downloadAudioUrl ||
              episode.originDownloadAudioUrl ||
              null;
            const downloadKey = `${episode.id}-audio`;
            const isCurrentItem =
              Boolean(currentEpisodeId) && episode.id === currentEpisodeId;
