...
type LegacyVideoAudioMode =
  "muxed" | "separate" | "pure_music" | "not_applicable";
...
export type ResultVideoAction =
  "direct-download" | "merge-then-download" | "browser-hls-download" | "hide";
...
export function getResultMediaActions({
  ...
  const isVideoHls =
    isHlsPlaylistUrl(originDownloadVideoUrl) ||
    isHlsPlaylistUrl(videoDownloadUrl);
  const isAudioHls =
    isHlsPlaylistUrl(originDownloadAudioUrl) ||
    isHlsPlaylistUrl(audioDownloadUrl);
...
export function shouldShowVideoDownloadButton(
  videoDownloadUrl: string | null | undefined,
): boolean {
...
function extractResultImageUrl(
  value: string | UnifiedParseResultImage | null | undefined,
): string {
...
function normalizeResultImages(
  images?: Array<string | UnifiedParseResultImage> | null,
): string[] {
...
export function shouldUseFrontendImageProxy(
