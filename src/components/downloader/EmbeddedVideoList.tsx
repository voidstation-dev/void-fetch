...
export function EmbeddedVideoList({
  ...
  const {
    ...
    filteredVideos,
    autoScrollIndex >= 0
      ? Math.max(DEFAULT_VISIBLE_PARTS, autoScrollIndex + 1)
  ...
  useEffect(() => {
    ...
    setMobileVisibleCount((previous) =>
      Math.max(previous, autoScrollIndex + 1),
    );
      ...
      >
        <div className="space-y-2 pr-2">
          ...
          {visibleVideos.map((video, index) => {
            ...
            const isCurrentItem =
              Boolean(currentItemId) && video.id === currentItemId;
            const downloadActionCount =
              Number(shouldShowVideoDownloadButton(videoDownloadUrl)) +
              Number(Boolean(audioDownloadUrl));
              ...
              >
                <div className="flex w-full items-start gap-2 min-w-0 overflow-hidden">
                  ...
                  <div className="flex w-full items-center gap-2 flex-1 min-w-0 overflow-hidden">
                    <div
                      className="text-[13px] truncate min-w-0 flex-1 max-w-[64vw] sm:max-w-none"
                      title={displayTitle}
                    >
                      {displayTitle}
                    </div>
                    ...
                    >
                      {shouldShowVideoDownloadButton(videoDownloadUrl) && (
                        <MediaActionIconButton
                          ...
                          className="w-full"
                          onClick={() =>
                            triggerDownload(videoDownloadUrl!, videoKey)
                          }
                        />
                      )}
                      {audioDownloadUrl && (
                        <MediaActionIconButton
                          ...
                          className="w-full"
                          onClick={() =>
                            triggerDownload(audioDownloadUrl, audioKey)
                          }
                        />
                      )}
                    </div>
