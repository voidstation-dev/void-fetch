...
export async function selectCustomDirectory(): Promise<string | null> {
  if (!isDirectoryPickerSupported()) {
    throw new Error(
      "Your browser does not support custom directory selection. Download files will use standard browser downloads.",
    );
  }
  ...
  try {
    ...
    if (err instanceof DOMException) {
      ...
      if (err.name === "SecurityError") {
        throw new Error(
          "Chrome/Browser blocks selecting root system folders (like root Downloads) for security. Please pick a subfolder inside Downloads (e.g. Downloads/VoidFetch) or any custom folder on your drive.",
        );
      }
    }
    throw err;
...
export async function saveDownloadedFile(
  blob: Blob,
  filename: string,
): Promise<{ savedToCustomDir: boolean }> {
  if (activeDirectoryHandle) {
    const success = await saveBlobToCustomDirectory(
