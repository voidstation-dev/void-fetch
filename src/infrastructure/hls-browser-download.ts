...
export function pickBestVariant(
  ...
  const variants = (parsed.variants || [])
    .filter(
      (variant) => typeof variant.uri === "string" && variant.uri.length > 0,
    )
    .map((variant) => ({
  ...
  return (
    variants.sort((left, right) => right.bandwidth - left.bandwidth)[0] || null
  );
}
...
export function parseHlsMediaPlaylist(
  ...
  if (parsed.isMasterPlaylist) {
    throw new Error(
      "Expected an HLS media playlist but received a master playlist",
    );
  }

  const segments = (parsed.segments || [])
    .filter(
      (segment) => typeof segment.uri === "string" && segment.uri.length > 0,
    )
    .map((segment) => {
      ...
      return {
        ...
        keyUrl:
          method === "AES-128" && segment.key?.uri
            ? new URL(segment.key.uri, playlistUrl).toString()
            : undefined,
        iv:
          method === "AES-128"
            ? normalizeIv(segment.key?.iv) ||
              buildSequenceIv(segment.mediaSequenceNumber || 0)
            : undefined,
  ...
  if (segments.length === 0) {
    throw new Error(
      "The resolved playlist does not contain downloadable segments",
    );
  }
  ...
  return {
    mapUrl: firstMap?.uri
      ? new URL(firstMap.uri, playlistUrl).toString()
      : null,
...
export function shouldBlockLargeHlsDownloadWithoutStreamingSave(
  ...
  return (
    !supportsStreamingSave && segmentCount > NON_STREAMING_BROWSER_MAX_SEGMENTS
  );
}
...
export function buildHlsHostProbeTargets(
  ...
  const candidates = [
    ...(mapUrl ? [{ url: mapUrl, byterange: mapByterange }] : []),
...
export function sliceHlsSegments(
  segments: HlsSegment[],
  limit?: number,
): HlsSegment[] {
