...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  try {
    targetUrl = parseHttpUrl(
      request.nextUrl.searchParams.get("target"),
      "target",
    );
    refererUrl = parseHttpUrl(
  ...
  const likelyPlaylistRequest =
    isHlsPlaylistUrl(targetUrl.toString()) ||
  ...
  const shouldRewritePlaylist = isHlsPlaylistResponse(
    targetUrl.toString(),
    contentType,
  );
