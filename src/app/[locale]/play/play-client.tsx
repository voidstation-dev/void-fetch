...
const AceternityGridBackground = ({ coverUrl }: { coverUrl?: string | null }) => {
    return (
                    ...
                    {loading ? (
                        <div className="aspect-video max-h-[75dvh] flex items-center justify-center text-sm text-indigo-400/80">
                            <div className="flex flex-col items-center gap-4">
                                ...
                                <span className="tracking-[0.2em] font-mono uppercase text-[10px] opacity-70">{tForm('downloading')}</span>
                            </div>
                        </div>
                    ) : (canPlay && playbackUrl) ? (
                        ...
                        <div className="aspect-video max-h-[75dvh] flex items-center justify-center px-4 text-sm text-slate-400 bg-black/80">
                            <div className="flex flex-col items-center gap-3 p-6 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md">
                                ...
                                <span className="font-medium tracking-wide">{tResult('sharePlayUnavailable')}</span>
                            </div>
                        </div>
                    ) : null}
                </motion.section>
                        ...
                        {!loading && displayError && (
                            <Card className="bg-red-950/30 border-red-900/50 backdrop-blur-xl shadow-xl">
                                <CardContent className="p-6 space-y-4">
                                    ...
                                    <Button asChild size="sm" variant="outline" className="border-red-900/50 hover:bg-red-900/30 text-red-200">
                        ...
                        {!loading && !displayError && visibleParseResult && (
                            <div className="w-full space-y-4 p-6 md:p-8 rounded-[2rem] bg-slate-900/40 border border-white/5 backdrop-blur-2xl shadow-xl relative overflow-hidden group">
                                ...
                                <div className="flex items-center gap-3 mb-2 relative z-10">
                                    ...
                                    <span className="text-[10px] uppercase tracking-[0.2em] font-mono text-indigo-300/80 font-bold">Shared Media</span>
                                </div>
                                <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl leading-tight font-bold wrap-break-word text-white tracking-tight relative z-10" title={visibleParseResult.title}>
                                ...
                                <div className="flex flex-wrap items-center gap-3 text-xs text-slate-400 pt-2 relative z-10">
                                    {canonicalSourceUrl ? (
                                        ...
                                        >
                                            ...
                                            <span className="font-medium tracking-wide">{tHistory('viewSource')}</span>
                                        </a>
                                    ) : null}
                                </div>
